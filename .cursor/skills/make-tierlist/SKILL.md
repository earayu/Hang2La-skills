...
### 第 1 步 — 规划选手与排名（Agent 完成）

根据话题，选取 5–15 个有代表性的品类/角色/事物作为"选手"。为每位选手：

1. **写点评文案**（口语化、幽默、有共情点，每人 100–400 字，朗读约 30–120 秒）。
2. **定档位**（从 hang / top / rsr / npc / la 中选一个）。
3. **写 2–4 个搜图关键词**（具体名词，避免动词）。
...
contestants:
  - index: 1
    name: "帝王牧绅一"
    keywords: "Akira Sendoh basketball anime Slam Dunk"
    tier: "hang"
    text: |
      首先是帝王牧绅一，30分且三双达成。
      整场比赛，阿牧的表现无可挑剔...
  - index: 2
    name: "赤木刚宪"
    keywords: "Takenori Akagi basketball anime Slam Dunk center"
    tier: "hang"
    text: |
      接下来是湘北队长赤木...
```

**参考时长**：约 120 汉字 ≈ 8–10 秒音频。
...
python .cursor/skills/search-images/scripts/search_images.py \
  ...
  --output "projects/{name}/images/contestant_{index:02d}/" \
  --count 3 \
  --config config.yaml
```

同时为 `intro` 和 `outro`（如需配图）搜图。

更新 `project.yaml`：将 `image` 设为返回结果中第一张图片的 `path`。

> **注意**：选手图片将在视频中大图展示（约 720×720），建议选择构图清晰、主体明显的图片。

---

### 第 3 步 — 生成音频

对 `intro`、每位 `contestant`、`outro` 分别执行：
...
更新 `project.yaml`：填写每段的 `audio` 和 `duration` 字段。
...
| `contestants[].keywords` | 搜图关键词 |
| `contestants[].text` | 点评旁白文案 |
...
```
- [ ] 第 0 步：project.yaml 已创建，type = "tierlist"
- [ ] 第 1 步：所有选手均有 text、keywords、tier；intro/outro 有 text
- [ ] 第 2 步：所有选手均有 image（下载或用户提供）
